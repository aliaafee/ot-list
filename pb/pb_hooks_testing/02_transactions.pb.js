...
routerAdd('POST', '/api/add-procedure-with-patient', (e) => {
    ...
    if (!data.procedure.procedureDay) {
        throw new BadRequestError('Missing required field: procedure.procedureDay');
    }

    if (!data.procedure.operatingRoom) {
        throw new BadRequestError('Missing required field: procedure.operatingRoom');
    }
    ...
    try {
        $app.runInTransaction((txApp) => {
            ...
            if (data.patient && data.patient.id) {
                createdPatient = txApp.findRecordById('patients', data.patient.id);
            } else if (data.patient) {
                const patientCollection = txApp.findCollectionByNameOrId('patients');
                ...
                console.log(`[add-procedure-with-patient] Created patient: ${patientRecord.id}`);
            } else {
                throw new BadRequestError('Missing required field: patient');
            }

            // Create procedure
            const procedureCollection = txApp.findCollectionByNameOrId('procedures');
            ...
            console.log(`[add-procedure-with-patient] Created procedure: ${procedureRecord.id}`);
        });

        // Fetch expanded procedure for response
        const expandedProcedure = $app.findRecordById('procedures', createdProcedure.id);
        $app.expandRecord(expandedProcedure, ['patient', 'addedBy', 'procedureDay', 'creator', 'updater'], null);
        ...
        console.error('[add-procedure-with-patient] Transaction error:', error);
        throw new BadRequestError(`Failed to add procedure with patient: ${error.message}`);
    }
}, $apis.requireAuth());
...
routerAdd('POST', '/api/add-pac-status', (e) => {
    const authRecord = e.auth;
    if (!authRecord) {
        throw new UnauthorizedError('Authentication required');
    }
    ...
    if (!validStatuses.includes(data.pacStatus)) {
        throw new BadRequestError(`Invalid pacStatus. Must be one of: ${validStatuses.join(', ')}`);
    }
    ...
    try {
        $app.runInTransaction((txApp) => {
            const statusCollection = txApp.findCollectionByNameOrId('procedurePacStatuses');
            ...
            statusRecord.set('updater', authRecord.id);

            txApp.save(statusRecord);
            createdStatus = statusRecord;
            console.log(`[add-pac-status] Created PAC status: ${statusRecord.id}`);

            const procedure = txApp.findRecordById('procedures', data.procedureId);
            procedure.set('pacStatus', data.pacStatus);
            procedure.set('updater', authRecord.id);
            txApp.save(procedure);
            updatedProcedure = procedure;
            console.log(`[add-pac-status] Updated procedure pacStatus: ${data.procedureId}`);
        });
        ...
        throw new BadRequestError(`Failed to add PAC status: ${error.message}`);
    }
}, $apis.requireAuth());
...
routerAdd('POST', '/api/bulk-update-procedures', (e) => {
    const authRecord = e.auth;
    if (!authRecord) {
        throw new UnauthorizedError('Authentication required');
    }
    ...
    if (!data.procedures || !Array.isArray(data.procedures)) {
        throw new BadRequestError('Missing required field: procedures (array)');
    }
    ...
    try {
        $app.runInTransaction((txApp) => {
            data.procedures.forEach((procedureUpdate) => {
                if (!procedureUpdate.id) {
                    throw new BadRequestError('Each procedure must have an id');
                }
                ...
                console.log(`[bulk-update-procedures] Updated procedure: ${record.id}`);
            });
        });
        ...
        throw new BadRequestError(`Failed to bulk update procedures: ${error.message}`);
    }
}, $apis.requireAuth());
